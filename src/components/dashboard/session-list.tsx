...
export default function SessionList({
    ...
    if (statusFilter !== "all") {
      if (statusFilter === "pushed") {
        result = result.filter(
          (s) => s.status === "pushed" || s.status === "partially_pushed",
        );
      } else {
  ...
  async function bulkDelete() {
    ...
    setSessions((prev) =>
      prev.filter((s) => !ids.includes(s.id) || failed.includes(s.id)),
    );
          ...
          {filterOptions.map((f) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg cursor-pointer transition-all whitespace-nowrap ${
                  isActive
              ...
              >
                {f.label}
                <span
                  className={`text-[10px] font-bold px-1 py-px rounded-md ${
                    isActive
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((session) => {
            ...
            const InputIcon =
              inputTypeIcon[session.input_type as keyof typeof inputTypeIcon];
            ...
            return (
              <div
                key={session.id}
                className={`group flex items-center gap-3 bg-bg-card border rounded-[14px] p-3.5 transition-all ${
                  isSelected
              ...
              >
                ...
                <button
                  ...
                  className={`w-5 h-5 rounded flex items-center justify-center shrink-0 cursor-pointer transition-colors ${
                    isSelected
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-text truncate">
                        {session.title ||
                          `${session.input_type.charAt(0).toUpperCase() + session.input_type.slice(1)} Upload`}
                      </span>
                      <span
