...
const GLOBAL_DAILY_PARSE_LIMIT = parseInt(
  process.env.GLOBAL_DAILY_PARSE_LIMIT || "400",
);
...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      profile.monthly_parse_count >= parseLimit
    ) {
      return NextResponse.json(
        {
          error:
            "Monthly parse limit reached. Add your own AI API key in Settings for unlimited use.",
        },
        { status: 429 },
      );
    }
    ...
    if (!profile.custom_ai_api_key) {
      ...
      if (!globalCheck.allowed) {
        return NextResponse.json(
          {
            error:
              "Daily server limit reached. Please try again tomorrow or add your own AI API key for unlimited use.",
          },
          { status: 429 },
        );
      }
    ...
    if (
      !["pdf", "image", "text"].includes(inputType) ||
