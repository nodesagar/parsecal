...
export default function ReviewPage() {
  ...
  function startEdit(event: ParsedEvent) {
    ...
    setCustomRuleMode(
      !!event.recurrence_rule && !presets.includes(event.recurrence_rule),
    );
    setEditValues({
      title: event.title,
  ...
  async function saveEdit(eventId: string) {
    const updatePayload: Partial<ParsedEvent> & { is_edited: boolean } = {
    ...
    setEvents((prev) =>
      prev.map((e) =>
        e.id === eventId ? ({ ...e, ...updatePayload } as ParsedEvent) : e,
      ),
    );
    setEditingId(null);
  }
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-text-muted">
            ...
            {alreadyPushedCount > 0 &&
              ` • ${alreadyPushedCount} already pushed`}
          </p>
        </div>
      </div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <button
                    ...
                    className={`w-5 h-5 rounded-[4px] border-2 flex items-center justify-center mt-0.5 flex-shrink-0 cursor-pointer ${
                      event.is_selected
                    ...
                    {isEditing ? (
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className={`relative w-8 h-[18px] rounded-full transition-colors cursor-pointer ${
                              editValues.is_all_day ? "bg-primary" : "bg-border"
                            }`}
                          >
                            <span
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              value={
                                editValues.is_all_day
                                  ? (editValues.start_datetime || "").slice(
                                      0,
                                      10,
                                    )
                                  : (editValues.start_datetime || "").slice(
                            ...
                            <button
                              ...
                              onClick={() =>
                                setEditValues({
                                  ...editValues,
                                  is_recurring: !editValues.is_recurring,
                                  recurrence_rule: !editValues.is_recurring
                          ...
                          {editValues.is_recurring && (
                            <div className="space-y-2">
                                  ...
                                  customRuleMode
                                    ...
                                    : editValues.recurrence_rule &&
                                        [
                                          ...
                                          "RRULE:FREQ=BIWEEKLY",
                                          "RRULE:FREQ=MONTHLY",
                                        ].includes(editValues.recurrence_rule)
                                      ? editValues.recurrence_rule
                                      : editValues.recurrence_rule
                                        ? "custom"
                                        : ""
                              ...
                              >
                                ...
                                <option value="RRULE:FREQ=WEEKLY">
                                  Weekly
                                </option>
                                <option value="RRULE:FREQ=BIWEEKLY">
                                  Every 2 weeks
                                </option>
                                <option value="RRULE:FREQ=MONTHLY">
                                  Monthly
                                </option>
                      ...
                      <>
                        <div className="flex items-center gap-2 mb-1">
                          <h3
                            className={`text-sm font-semibold truncate ${event.pushed_at ? "text-success" : "text-text"}`}
                          >
                            {event.title}
                          </h3>
          ...
          >
            {pushing ? (
              ...
              <>
                ...
                {selectedProvider === "ics"
                  ? `Export ${selectedCount} Event${selectedCount !== 1 ? "s" : ""}`
                  : `Push ${selectedCount} Event${selectedCount !== 1 ? "s" : ""}`}
              </>
            )}
          </button>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold">
                Successfully pushed {pushResult.successful} events!
              </p>
              {pushResult.failed > 0 && (
                <p className="text-sm text-error">
                  {pushResult.failed} events failed to push.
                </p>
              )}
            </div>
          </div>
